...
var consumerNewsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		rmq.SetUpChannel(RMQ_EXCHANGE_TYPE, true, RMQ_SMS_ACTU_EXCHANGE, true, RMQ_SMS_ACTU_QUEUE)
...
var consumerSMSActuCmd = &cobra.Command{
	Use:   "sms_actu",
	Short: "Consumer SMS Actu Service CLI",
	Long:  ``,
	Run: func(cmd *cobra.Command, args []string) {
		/**
		 * connect mysql
		...
		messagesData, errSub := rmq.Subscribe(1, false, RMQ_SMS_ACTU_QUEUE, RMQ_SMS_ACTU_EXCHANGE, RMQ_SMS_ACTU_QUEUE)
		if errSub != nil {
			panic(errSub)
		}
