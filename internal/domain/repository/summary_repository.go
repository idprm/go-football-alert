...
func (r *SummaryTotalDailyRepository) SelectTotalDaily() (*[]entity.SummaryTotalDaily, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.CreatedAt, &s.TotalSub, &s.TotalUnsub, &s.TotalRenewal, &s.TotalRevenue); err != nil {
			return nil, err
		}
		summs = append(summs, s)
	}
